...
const HeartIcon = ({ filled }) => (
  <svg className="w-5 h-5 sm:w-6 sm:h-6 transition-transform duration-150 group-active:scale-75" viewBox="0 0 24 24" fill={filled ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={filled ? 0 : 2}>
...
const ProjectCard = ({ project, onLikeChange }) => {
  ...
  useEffect(() => {
    const syncLikeState = async () => {
      ...
      try {
        const promises = [getLikeCount(project._id)];
        if (isRecruiter) promises.push(getLikeStatus(project._id));
        
        const results = await Promise.all(promises);
        setLikeCount(results[0].data?.count ?? 0);
        if (isRecruiter) setLiked(Boolean(results[1].data?.liked));
      } catch (err) {
      ...
      <div className="flex flex-col flex-1 px-5 pt-4 pb-5 gap-3">
        ...
        <div className="flex items-center justify-between gap-3 pt-3 border-t border-slate-50 mt-auto">
          ...
          <button
            ...
            className={`group relative flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold transition-all duration-200 select-none focus:outline-none shrink-0 ${liked
              ? 'text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-100'
              : isRecruiter
                ? 'text-slate-500 bg-slate-50 hover:bg-rose-50 hover:text-rose-500 border border-slate-200 hover:border-rose-100'
                : 'text-slate-600 bg-slate-100 border border-slate-200 cursor-default'
              } disabled:opacity-60`}
          >
            <span className={`${liked ? 'text-rose-500' : isRecruiter ? 'group-hover:text-rose-400' : 'text-slate-500'} transition-colors`}>
